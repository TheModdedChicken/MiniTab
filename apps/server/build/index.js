...
var require_node2 = __commonJS((exports) => {
  var isTag = function(node4) {
    return (0, domelementtype_1.isTag)(node4);
  };
  var isCDATA = function(node4) {
    return node4.type === domelementtype_1.ElementType.CDATA;
  };
  var isText = function(node4) {
    return node4.type === domelementtype_1.ElementType.Text;
  };
  var isComment = function(node4) {
    return node4.type === domelementtype_1.ElementType.Comment;
  };
  var isDirective = function(node4) {
    return node4.type === domelementtype_1.ElementType.Directive;
  };
  var isDocument = function(node4) {
    return node4.type === domelementtype_1.ElementType.Root;
  };
  var hasChildren = function(node4) {
    return Object.prototype.hasOwnProperty.call(node4, "children");
  };
  var cloneNode = function(node4, recursive) {
    if (recursive === undefined) {
      recursive = false;
    }
    var result;
    if (isText(node4)) {
      result = new Text(node4.data);
    } else if (isComment(node4)) {
      result = new Comment(node4.data);
    } else if (isTag(node4)) {
      var children = recursive ? cloneChildren(node4.children) : [];
      var clone_1 = new Element(node4.name, __assign({}, node4.attribs), children);
      children.forEach(function(child) {
        return child.parent = clone_1;
      });
      if (node4.namespace != null) {
        clone_1.namespace = node4.namespace;
      }
      if (node4["x-attribsNamespace"]) {
        clone_1["x-attribsNamespace"] = __assign({}, node4["x-attribsNamespace"]);
      }
      if (node4["x-attribsPrefix"]) {
        clone_1["x-attribsPrefix"] = __assign({}, node4["x-attribsPrefix"]);
      }
      result = clone_1;
      ...
      if (node4["x-mode"]) {
        clone_3["x-mode"] = node4["x-mode"];
      }
      result = clone_3;
    } else if (isDirective(node4)) {
      var instruction = new ProcessingInstruction(node4.name, node4.data);
      ...
      throw new Error("Not implemented yet: ".concat(node4.type));
    }
  ...
  var DomHandler = function() {
    ...
    DomHandler2.prototype.ontext = function(data) {
      ...
      if (lastNode && lastNode.type === domelementtype_1.ElementType.Text) {
        ...
        var node4 = new node_js_1.Text(data);
        this.addNode(node4);
        this.lastNode = node4;
      }
    };
    DomHandler2.prototype.oncomment = function(data) {
      ...
      var node4 = new node_js_1.Comment(data);
      this.addNode(node4);
      this.lastNode = node4;
    };
    ...
    DomHandler2.prototype.oncdatastart = function() {
      ...
      var node4 = new node_js_1.CDATA([text]);
      this.addNode(node4);
      text.parent = node4;
      this.lastNode = text;
    };
    ...
    DomHandler2.prototype.onprocessinginstruction = function(name, data) {
      var node4 = new node_js_1.ProcessingInstruction(name, data);
      this.addNode(node4);
    };
    ...
    DomHandler2.prototype.addNode = function(node4) {
...
var require_lib4 = __commonJS((exports) => {
  ...
  var render = function(node4, options) {
    if (options === undefined) {
      options = {};
    }
    var nodes = "length" in node4 ? node4 : [node4];
    var output = "";
...
var require_stringify = __commonJS((exports) => {
  var getOuterHTML = function(node4, options) {
    return (0, dom_serializer_1.default)(node4, options);
  };
  var getInnerHTML = function(node4, options) {
    return (0, domhandler_1.hasChildren)(node4) ? node4.children.map(function(node5) {
      return getOuterHTML(node5, options);
    }).join("") : "";
  };
  var getText = function(node4) {
    if (Array.isArray(node4))
      return node4.map(getText).join("");
    if ((0, domhandler_1.isTag)(node4))
      return node4.name === "br" ? "\n" : getText(node4.children);
    if ((0, domhandler_1.isCDATA)(node4))
      return getText(node4.children);
    if ((0, domhandler_1.isText)(node4))
      return node4.data;
    return "";
  };
  var textContent = function(node4) {
    if (Array.isArray(node4))
      return node4.map(textContent).join("");
    if ((0, domhandler_1.hasChildren)(node4) && !(0, domhandler_1.isComment)(node4)) {
  ...
  var innerText = function(node4) {
    if (Array.isArray(node4))
      return node4.map(innerText).join("");
    if ((0, domhandler_1.hasChildren)(node4) && (node4.type === domelementtype_1.ElementType.Tag || (0, domhandler_1.isCDATA)(node4))) {
...
var require_querying = __commonJS((exports) => {
  var filter = function(test, node4, recurse, limit) {
...
var require_legacy = __commonJS((exports) => {
  ...
  var testElement = function(options, node4) {
...
var require_helpers = __commonJS((exports) => {
  var removeSubsets = function(nodes) {
    ...
    while (--idx >= 0) {
      ...
      for (var ancestor = node4.parent;ancestor; ancestor = ancestor.parent) {
  ...
  var uniqueSort = function(nodes) {
    nodes = nodes.filter(function(node4, i, arr) {
      return !arr.includes(node4, i + 1);
    });
...
var require_feeds = __commonJS((exports) => {
  ...
  var getOneElement = function(tagName, node4) {
    return (0, legacy_js_1.getElementsByTagName)(tagName, node4, true, 1)[0];
  };
...
var require_subselects = __commonJS((exports) => {
  ...
  var is = function(next, token, options, context3, compileToken) {
    var func = compileToken(token, copyOptions(options), context3);
  ...
  exports.subselects = {
    ...
    has: function(next, subselect, options, _context, compileToken) {
      ...
      if (context3 && compiled !== boolbase_1.default.trueFunc) {
...
var require_pseudo_selectors = __commonJS((exports) => {
  var compilePseudoSelector = function(next, selector, options, context3, compileToken) {
    var _a;
    ...
    if (Array.isArray(data)) {
      ...
      return subselects_js_1.subselects[name](next, data, options, context3, compileToken);
    }
    ...
    if (typeof stringPseudo === "string") {
      ...
      return subselects_js_1.subselects["is"](next, alias, options, context3, compileToken);
    }
    ...
    if (name in filters_js_1.filters) {
      return filters_js_1.filters[name](next, data, options, context3);
    }
...
var require_general = __commonJS((exports) => {
  var getElementParent = function(node4, adapter) {
    var parent = adapter.getParent(node4);
    if (parent && adapter.isTag(parent)) {
      return parent;
    }
    return null;
  };
  var compileGeneralSelector = function(next, selector, options, context3, compileToken) {
    ...
    switch (selector.type) {
      ...
      case css_what_1.SelectorType.Pseudo: {
        return (0, index_js_1.compilePseudoSelector)(next, selector, options, context3, compileToken);
      }
...
var require_compile2 = __commonJS((exports) => {
  var compile = function(selector, options, context3) {
    var next = compileUnsafe(selector, options, context3);
    return (0, subselects_js_1.ensureIsTag)(next, options.adapter);
  };
  var compileUnsafe = function(selector, options, context3) {
  ...
  var absolutize = function(token, _a, context3) {
    var adapter = _a.adapter;
    var hasContext = !!(context3 === null || context3 === undefined ? undefined : context3.every(function(e) {
  ...
  var compileToken = function(token, options, context3) {
    var _a;
    token.forEach(sort_js_1.default);
    context3 = (_a = options.context) !== null && _a !== undefined ? _a : context3;
  ...
  var compileRules = function(rules, options, context3) {
    var _a;
    return rules.reduce(function(previous, rule) {
      return previous === boolbase_1.default.falseFunc ? boolbase_1.default.falseFunc : (0, general_js_1.compileGeneralSelector)(previous, rule, options, context3, compileToken);
...
var require_matcher = __commonJS((exports) => {
  var isTag = function(node4) {
    return node4 && node4.nodeType === type_1.default.ELEMENT_NODE;
  };
  var getAttributeValue = function(elem, name) {
  ...
  var getChildren = function(node4) {
    return node4 && node4.childNodes;
  };
  var getParent = function(node4) {
    return node4 ? node4.parentNode : null;
  };
  var getText = function(node4) {
    return node4.text;
  };
  ...
  var getSiblings = function(node4) {
    var parent = getParent(node4);
...
var require_html = __commonJS((exports) => {
  ...
  var resetParent = function(nodes, parent) {
    return nodes.map(function(node4) {
      node4.parentNode = parent;
      return node4;
    });
  };
  var __extends = exports && exports.__extends || function() {
  ...
  var HTMLElement = function(_super) {
    ...
    HTMLElement2.prototype.removeChild = function(node4) {
    ...
    Object.defineProperty(HTMLElement2.prototype, "structuredText", {
      get: function() {
        ...
        function dfs(node4) {
          if (node4.nodeType === type_1.default.ELEMENT_NODE) {
            if (kBlockElements.has(node4.rawTagName)) {
              if (currentBlock.length > 0) {
                blocks.push(currentBlock = []);
              }
              node4.childNodes.forEach(dfs);
              if (currentBlock.length > 0) {
                blocks.push(currentBlock = []);
              }
            } else {
          ...
          } else if (node4.nodeType === type_1.default.TEXT_NODE) {
            if (node4.isWhitespace) {
              currentBlock.prependWhitespace = true;
            } else {
              var text = node4.trimmedText;
    ...
    HTMLElement2.prototype.replaceWith = function() {
      ...
      var content = nodes.map(function(node4) {
        if (node4 instanceof node_1.default) {
          return [node4];
        } else if (typeof node4 == "string") {
          var r = parse2(node4, _this._parseOptions);
          return r.childNodes.length ? r.childNodes : [new text_1.default(node4, _this)];
        }
        return [];
      }).flat();
    ...
    Object.defineProperty(HTMLElement2.prototype, "structure", {
      get: function() {
        ...
        function dfs(node4) {
          var idStr = node4.id ? "#".concat(node4.id) : "";
          var classStr = node4.classList.length ? ".".concat(node4.classList.value.join(".")) : "";
          write("".concat(node4.rawTagName).concat(idStr).concat(classStr));
          indention++;
          node4.childNodes.forEach(function(childNode) {
            if (childNode.nodeType === type_1.default.ELEMENT_NODE) {
              dfs(childNode);
    ...
    HTMLElement2.prototype.removeWhitespace = function() {
      var _this = this;
      var o = 0;
      this.childNodes.forEach(function(node4) {
        if (node4.nodeType === type_1.default.TEXT_NODE) {
          if (node4.isWhitespace) {
            return;
          }
          node4.rawText = node4.trimmedRawText;
        } else if (node4.nodeType === type_1.default.ELEMENT_NODE) {
          node4.removeWhitespace();
        }
        _this.childNodes[o++] = node4;
      });
    ...
    HTMLElement2.prototype.closest = function(selector) {
      ...
      while (el) {
        var e = (0, css_select_1.selectOne)(selector, el, {
          xmlMode: true,
          adapter: __assign(__assign({}, matcher_1.default), { getChildren: function(node4) {
            var child = mapChild.get(node4);
            return child && [child];
          }, getSiblings: function(node4) {
            return [node4];
    ...
    HTMLElement2.prototype.appendChild = function(node4) {
      node4.remove();
      this.childNodes.push(node4);
      node4.parentNode = this;
      return node4;
    };
...
var trimTrailingSlash = () => {
  return async function trimTrailingSlash2(c, next) {
    ...
    if (c.res.status === 404 && c.req.method === "GET" && c.req.path !== "/" && c.req.path[c.req.path.length - 1] === "/") {
      const url6 = new URL(c.req.url);
      url6.pathname = url6.pathname.substring(0, url6.pathname.length - 1);
      c.res = c.redirect(url6.toString(), 301);
    }
  };
};
...
var zValidator = (target, schema2, hook) => validator(target, async (value, c) => {
...
function formatTabType(type) {
  return type.replace("Ukulele Chords", "Ukulele").replace("Bass Tabs", "Bass");
}
var TabType;
(function(TabType2) {
  TabType2[TabType2["Tab"] = 200] = "Tab";
  TabType2[TabType2["Chords"] = 300] = "Chords";
  TabType2[TabType2["Bass"] = 400] = "Bass";
  TabType2[TabType2["Ukulele"] = 800] = "Ukulele";
})(TabType || (TabType = {}));
...
// src/utility/fetch.ts
async function getPage(url7, details = false) {
  const res = await fetch(url7);
  if (!res.ok)
    throw new Error("Failed to fetch page");
  const html2 = import_node_html_parser.default(await res.text());
  return {
    html: html2,
    details: details ? getPageDetails(html2) : undefined
  };
}
function getPageDetails(html2) {
  const element = html2.querySelector(".js-store[data-content]");
  if (!element)
    return null;
  const raw2 = element.getAttribute("data-content");
  if (!raw2)
    return null;
  const flattened = import_node_html_parser.default(raw2).innerText;
  const parsed = JSON.parse(flattened);
  return parsed;
}
async function getSuggestions(search) {
  const keyword = search.toLowerCase().split(" ")[0];
  const res = await fetch("https://www.ultimate-guitar.com/static/article/suggestions" + `/${keyword[0]}` + `/${keyword.substring(0, 5)}.js`);
  try {
    const data = await res.json();
    if (data.suggestions)
      return data.suggestions.filter((v) => v.includes(keyword.toLowerCase())).splice(0, 5);
  } catch (e) {
  }
  return [];
}
async function getTabs(search) {
  ...
  const tabs = results.filter((result) => !result.marketing_type && Object.keys(TabType).includes(formatTabType(result.type))).map((result) => ({
...
async function getTab(slug) {
  ...
  const {
    ...
    votes,
    type
  } = details.store.page.data.tab;
  const tuning = tab_view?.meta?.tuning?.value?.split(" ") || ["E", "A", "D", "G", "B", "E"];
  const difficulty = tab_view?.ug_difficulty || "unknown";
  const raw_tabs = tab_view?.wiki_tab?.content || "";
  const chordsDiagrams = tab_view?.applicature || [];
  const versions = tab_view?.versions.filter((tab) => tab.type !== "Official") || [];
  let versionsFormatted = versions.map((tabScrapped) => {
    return {
  ...
  return {
    ...
    chordsDiagrams,
    htmlTab: `<pre>${tab_view.wiki_tab.content}</pre>`.replaceAll("[ch]", '<span class="text-chord">').replaceAll("[/ch]", "</span>").replaceAll("[tab]", "<span>").replaceAll("[/tab]", "</span>")
  };
}

// src/routes.ts
var Routes = new Hono2().basePath("/");
...
var routes_default = Routes.route("/tabs", new Hono2().on("SEARCH", "/", zValidator("json", SearchArgsSchema), async (c) => {
  const body2 = c.req.valid("json");
  return c.json(await getTabs(body2));
}).get("/:slug", async (c) => {
  const slug = c.req.param("slug");
  return c.json(await getTab(slug));
})).get("/suggestions", zValidator("query", z.object({
  q: z.string()
})), async (c) => {
  const { q } = c.req.valid("query");
  return c.json(await getSuggestions(q));
});

// src/index.ts
var app = new Hono2().basePath("/api");
var api = app.use(trimTrailingSlash()).use((c, next) => {
  c.header("Access-Control-Allow-Origin", "*");
  c.header("Access-Control-Allow-Credentials", "true");
  c.header("Access-Control-Allow-Headers", "Content-Type, Content-Length, Accept-Encoding, Accept, Origin, Cache-Control");
  c.header("Access-Control-Allow-Methods", "GET, SEARCH, POST, OPTIONS");
  return next();
}).route("/", routes_default);
var src_default = {
  port: env_default.PORT || 8000,
  fetch: app.fetch
};
